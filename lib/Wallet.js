...
if (typeof window != undefined) {
	...
	generateRandomBytes = (n) => {
		return crypto.lib.WordArray.random(n);
	}
	pbkdf2Sync = (pw, salt, options) => {
		return crypto.PBKDF2(Buffer.from(pw), Buffer.from(salt, 'hex'), {
			iterations: options.c,
			keySize: option.dklen,
			hasher: crypto.algo.SHA256
		});
	}
	createDecipher = (cipher, key, iv) => {
		return crypto.AES.decrypt(cipher, key, { iv: iv });
	}
} else {
	...
	pbkdf2Sync = (pw, salt, options) => {
		return crypto.pbkdf2Sync(Buffer.from(pw), Buffer.from(salt, 'hex'), options.c, options.dklen, 'sha256');
	} 
	createDecipher = (cipher, key, iv) => {
...
export default class Wallet {
	...
	static create() {
		...
		do {
			privKey = generateRandomBytes(32)();
			console.log(privKey)
	...
	static loadKeystore(keystore, password, nonStrict) {
		...
		if (json.crypto.kdf === 'scrypt') {
			...
			derivedKey = pbkdf2Sync(password, kdfparams.salt, {
				c: kdfparams.c,
		...
		const decipher = createDecipher(json.crypto.cipher, derivedKey.slice(0, 16), Buffer.from(json.crypto.cipherparams.iv, 'hex'))();
	...
	store(password, opts) {
		...
		opts = opts || {};
		const salt = opts.salt || generateRandomBytes(32)();
		const iv = opts.iv || generateRandomBytes(16)();
		...
		if (kdf === 'scrypt') {
			...
			derivedKey = pbkdf2Sync(password, salt, {
				c: kdfparams.c,
				dklen: kdfparams.dklen
			})(); 
			console.log(derivedKey)
		...
		return {
			version: 3,
			id: uuidv4({ random: opts.uuid || generateRandomBytes(16)() }),
